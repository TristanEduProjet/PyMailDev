...
class ParametersForm(Parameter, BaseWidget):
    ...
    json_path = json_path.join("resources\\parameters.json")"""

    def __init__(self):
        parameters = Parameter.__init__(self, '', '', '', '', '')
        BaseWidget.__init__(self, 'Email configuration')
        ...
        self._host = ControlText('Host')
        self._port = ControlText('Port')
        self._ssl = ControlText('SSL')
        self._user = ControlText('User')
        ...
        self.formset = ['_host', '_port', '_ssl', '_user', '_password', '_save', '_load']

        # Define the button actions
        self._save.value = self.__saveParamsAction
        self._load.value = self.__loadParamsAction
